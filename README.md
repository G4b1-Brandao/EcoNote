# ♻️ EcoNote – Reaproveitamento de Eletrônicos Descartados para o Ambiente Acadêmico  

Este repositório contém o desenvolvimento do projeto **EcoNote**, criado para a disciplina **Análise e Projeto de Sistemas**. O objetivo do projeto é promover a **inclusão digital** através do **reaproveitamento de notebooks e eletrônicos descartados**, conectando doadores a estudantes que necessitam desses equipamentos para seus estudos.  

## 👥 Equipe  
- **Engenharia de Requisitos:** Ana Gabrielle  
- **Modelagem de Sistema:** Marina Sena  
- **Desenvolvimento Frontend/Backend:** Ana Gabrielle e Paulo Henrique  
- **Testes:** Ana Gabrielle e Victor Souza  

## 🎯 Público-Alvo  
Nosso público-alvo são **estudantes do ensino superior e técnico do IFCE Campus Aracati** que enfrentam dificuldades financeiras para adquirir um notebook. Além disso, o projeto busca engajar **universidades, instituições de ensino, professores e alunos** que desejam contribuir com a inclusão digital por meio da **doação de equipamentos**.  

## 🚀 Como Funciona?  
- ✅ **Cadastro de Usuários** (Alunos e Professores) com validação por tipo de e-mail institucional  
- ✅ **Login com autenticação por grupo** (Aluno ou Professor)  
- ✅ **Cadastro e Gerenciamento de Notebooks** (feita por administradores)  
- ✅ **Solicitação de Notebooks** feita por alunos  
- ✅ **Avaliação das Solicitações** feita por professores  
- ✅ **Indicação de Alunos** para prioridade de recebimento  
- ✅ **Controle de Devoluções e Manutenções**  

> *Obs.: Administradores têm acesso privilegiado para gestão do sistema, mas não realizam cadastro direto na plataforma.*  


## 🔧 Tecnologias Utilizadas (a definir)  
- **Linguagem:** Python  
- **Framework:** Django  
- **Frontend:** HTML5, CSS3 (com Flexbox), Templates Django  
- **Banco de Dados:** SQLite3 (desenvolvimento)  
- **Hospedagem:** PythonAnywhere  
- **Controle de Versão:** Git + GitHub

  
## 📅 Status  
🟢 **Em desenvolvimento** 

## 🌱 Impacto Social e Ambiental

- 📚 Aumento da permanência estudantil por meio da inclusão digital  
- 🌍 Redução do descarte inadequado de notebooks 
- 🤝 Engajamento comunitário com foco em tecnologia e sustentabilidade  

## 📝 Contribuição  
Este repositório é mantido pelos integrantes da equipe para fins acadêmicos. Caso tenha sugestões ou queira acompanhar o desenvolvimento, fique à vontade para explorar o código e acompanhar os commits!  

---

📌 *Projeto desenvolvido para a disciplina de Análise e Projeto de Sistemas - 2025*  
 

